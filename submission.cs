                ...
                if (xr.NodeType == XmlNodeType.Element)
                {
                    switch (xr.Name)
                    {
                        case "Hotel":
                            ...
                            break;
                        case "Name":
                            curHotel.name = xr.ReadElementContentAsString();
                            break;
                        case "Phone":
                            curPhones.Add(xr.ReadElementContentAsString());
                            break;
                        case "Address":
                            curAddr.nearair = xr.GetAttribute("NearestAirport");
                            break;
                        case "Number":
                            curAddr.snumber = xr.ReadElementContentAsString();
                            break;
                        case "Street":
                            curAddr.street = xr.ReadElementContentAsString();
                            break;
                        case "City":
                            curAddr.city = xr.ReadElementContentAsString();
                            break;
                        case "State":
                            curAddr.state = xr.ReadElementContentAsString();
                            break;
                        case "Zip":
                            curAddr.zip = xr.ReadElementContentAsString();
                            break;
                    }
                }
                else if (xr.NodeType == XmlNodeType.EndElement && xr.Name == "Hotel")
                {
                    // End of a hotel element, add to the list
                    curHotel.phones = curPhones;
                    curHotel.addr = curAddr;
                    myHotels.Add(curHotel);
                }
